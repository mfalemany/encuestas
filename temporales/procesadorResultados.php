<?php 
	...
	class ProcesadorResultados{
		...
		private function agregar_observacion($obs){
			...
			if(strlen($obs) > 6 && 
				! preg_match("/no opin/i",$obs) && 
				! preg_match("/---/i",$obs) && 
				! preg_match("/\.\.\./i",$obs) &&
				! preg_match("/sin obse/i",trim($obs)) &&
				! preg_match("/no fue mi pro/i",trim($obs)) &&
				! preg_match("/ningu/i",trim($obs)) &&
				! preg_match("/\. \./i",trim($obs)) &&
				! preg_match("/no tuve clas/i",trim($obs)) 
				) { 
